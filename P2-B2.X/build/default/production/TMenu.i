...
const unsigned char ARROW[3] = {'-','>',' '};
const unsigned char OPTION1[20] = {"1. New Game"};
const unsigned char OPTION2[20] = {"2. Show top 5 scores"};
const unsigned char OPTION3[20] = {"3. Show users"};
const unsigned char OPTION4[20] = {"4. Modify users"};
const unsigned char OPTION5[20] = {"5. Show time"};
const unsigned char OPTION6[20] = {"6. Modify time"};

const unsigned char SELECT_USER[20] = {"1. Select User"};
const unsigned char NEW_USER[20] = {"2. New User"};
const unsigned char EDIT_NAME[20] = {"Edit Name"};
const unsigned char DELETE_USER[20] = {"Delete User"};
const unsigned char ENTER_NAME[20] = {"Enter Name:"};
const unsigned char ENTER_NEW_NAME[20] = {"Enter New Name"};
const unsigned char SHOW_TIME[20] = {"Current Time"};
const unsigned char ENTER_NEWTIME[20] = {"Enter New Time"};
const unsigned char TIME[20] = {"(HH:MM):"};
const unsigned char SCORE[20] = {"Score: "};
unsigned char HIGH_SCORE[20] = {"High score: "};
const unsigned char INFO_GAME[20] = {"T       | S "};

unsigned char pos, subMenu;
unsigned char mGame, mModUSelect, mModUEdit;
unsigned char time[5], timeAM[5];
...
void showMessage(unsigned char message[]){
    if (((subMenu == 1 && mGame == 1) || subMenu == 4) && playing == 0){
        if (message[posLCD - 3] != '\0' && (posLCD - 3) < 10){
...
void printOptions(unsigned char message1[], unsigned char message2[]){
    if (posLCD < 16){
        if (isPrinted == 0){
            ...
            LcGotoXY(posLCD, 1);
            if ((subMenu == 5 && posLCD > 5 - 1) || (subMenu == 3 && posLCD > 10)){
                LcPutChar(' ');
            } else {
            ...
            if(posMarkesine < 16){
                if (subMenu == 3 || subMenu == 2){
                    ...
                    if (message1[posMarkesine + sumand] != '\0' ) {
                        ...
                        } else if ((posMarkesine + sumand) <= 10){
...
void showLCD(unsigned char message1[], unsigned char message2[]){
    if ((message1[16] == '\0' && message2[16] == '\0' && subMenu != 3 && subMenu != 2) || playing == 1 || subMenu == 5) {
        printOptions(message1, message2);
    } else if (subMenu == 0){
        toMarkesination(message1, message2, 20, 3);
    } else if (subMenu == 2){
        toMarkesination(users1[0].name, HIGH_SCORE, 31, 0);
    } else if (subMenu == 3){
         toMarkesination(users1[0].name, users2[0].name, ((20 - 3) * ((numUsers >> 1) + 1)), 0);
    }
